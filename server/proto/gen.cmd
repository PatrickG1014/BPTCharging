...
CALL %PBTS_BIN_DIR%\pbjs -t static -w es6 trip.proto --no-create --no-encode --no-decode --no-verify --no-delimited -o %PBTS_OUT_DIR%\trip_pb_tmp.js
CALL ECHO import * as $protobuf from "protobufjs"; > %PBTS_OUT_DIR%\trip_pb.js
CALL ECHO. >> %PBTS_OUT_DIR%\trip_pb.js
CALL TYPE %PBTS_OUT_DIR%\trip_pb_tmp.js >> %PBTS_OUT_DIR%\trip_pb.js
CALL DEL %PBTS_OUT_DIR%\trip_pb_tmp.js
